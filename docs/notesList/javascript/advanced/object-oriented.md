...
但**构造函数的缺陷**在于，在构造函数内部定义的方法，会被重复创建，而不能在实例之间共享，也不能被子类所继承。所以通常实例属性定义在构造函数中，而实例方法需要定义在原型prototype上。两面性，prototype被实例共享，所以**原型对象/原型链的缺陷**在于定义在原型上面的引用属性，也会被实例共享，从而导致被污染的情况。
